...
def getBeta(train_x, train_y):
    ...
    #========================#
    rhs = train_x.T @ train_x
    rhs = np.linalg.inv(rhs)

    lhs = train_x.T @ train_y
...
def getBetaBatchGradient(train_x, train_y, lr, num_iter):
    ...
    for iter in range(0, num_iter):
        ...
        for i in range(n):
            ...
            # STRART YOUR CODE HERE  #
            #========================#
            mul = train_x[i].T @ beta
            inner = mul - train_y[i]
            element = train_x[i] * inner
            deriv = deriv + element
            #========================#
            #   END YOUR CODE HERE   #
            #========================#
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            ...
            #========================#
            beta = beta + \
                (lr * (train_y[idx] - (train_x[idx].T @ beta)) * train_x[idx])
            #========================#
            #   END YOUR CODE HERE   #
            #========================#
    return beta
...
class LinearRegression(object):
    ...
    def train(self, algType):
        ...
        if(algType == '0'):
            ...
            batch_lr = 0.05  # Adjust learning rate hyperparameter to 0.05, which seems to result in a better accuracy on normalized data
            beta = getBetaBatchGradient(
                newTrain_x, self.train_y.values, batch_lr, self.num_iter)
            print('Beta: ', beta)
        elif(algType == '2'):
            stochastic_lr = 0.0001  # Adjust learning rate hyperparameter to 0.0001, since lr = 0.005 does not converge for stochastic gradient descent on unnormalized data
            beta = getBetaStochasticGradient(
                newTrain_x, self.train_y.values, stochastic_lr)
            print('Beta: ', beta)
        else:
